...
export default function PlayerBoard({ player, students, compact = false, compactWidthClassName }: PlayerBoardProps) {
            ...
            <>
              <div className="absolute inset-1 rounded-md overflow-hidden">
                <Image
                  ...
                  alt={student.name}
                  fill
                  sizes={compact ? "(max-width: 640px) 12vw, 56px" : "(max-width: 640px) 15vw, 10vw"}
                  className="object-contain"
                />
              </div>
              <div className={`absolute rounded-full bg-black/70 uppercase text-white ${compact ? "top-1 left-1 px-1.5 py-0.5 text-[7px]" : "top-1 left-1 px-2 py-0.5 text-[8px]"}`}>
                {student.role === 'striker' ? 'ST' : 'SP'}
              </div>
            </>
          ) : (
            <div className="w-full h-full flex items-center justify-center text-white/20">
              <span className={compact ? "text-[8px]" : "text-[8px]"}>{label}</span>
            </div>
          )}
        </div>
        {!compact && student && (
  ...
  if (compact) {
    return (
      <div
        className={`${compactWidthClassName ?? "min-w-[220px] basis-[220px] sm:min-w-[236px] sm:basis-[236px] md:min-w-[252px] md:basis-[252px] lg:min-w-[280px] lg:basis-[280px]"} flex-shrink-0 p-4 bg-white/5 border border-white/10 rounded-xl`}
      ...
      >
        <div className="flex items-center justify-between mb-3 gap-3">
          <div className="flex items-center gap-2 min-w-0">
            ...
            {player.isHost && (
              <span className="flex-shrink-0 text-[10px] bg-yellow-500/20 text-yellow-500 border border-yellow-500/50 px-1.5 py-0.5 rounded">
                HOST
              </span>
            )}
          </div>
        ...
        <div className="flex w-full min-w-0 items-center gap-1.5" data-testid="compact-slot-row">
          <div className="flex min-w-0 flex-[4] gap-1.5">
